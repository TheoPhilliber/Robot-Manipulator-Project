...
CallbackReturn ArduinobotInterface::on_activate(const rclcpp_lifecycle::State & previous_state)
{
    ...
    {
        RCLCPP_FATAL_STREAM(rclcpp::get_logger("ArduinobotInterface"), "Something went wrong when interacting with the port " <<port_);
        return CallbackReturn::FAILURE;
    }
...
hardware_interface::return_type ArduinobotInterface::write(const rclcpp::Time & time, const rclcpp::Duration & period)
{
    ...
    try{
        ...
        RCLCPP_ERROR_STREAM(rclcpp::get_logger("ArduinobotInterface"), "Something went wrong while sending the message " << msg << " to the port " << port_);
